...
const listMagic = document.querySelector('#lista-magic');

function listarCards(listdatabase) {
    for (let i = 0; i < listdatabase.length; i++) {
        let card = listdatabase[i];

        if (card.tag[0] === "Pokémon") {
            criarCardPokemon(card);

        }
        else if (card.tag[0] === "Yu-Gi-Oh") {
            criarCardYugioh(card);

        }
        else if (card.tag[0] === "Magic") {
            criarCardMagic(card);
...
function criarCardPokemon(database) {
...
function criarCardMagic(database) {
    let idTag = database.id;
    let categoria = database.tag;
    let cardNome = database.nameCard;
    let cardImg = database.img;
...
botaoRemoverCard.forEach(botaoRemover => {
